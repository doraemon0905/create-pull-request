            ...
            while (retryAttempt <= maxRetries) {
                try {
                    ...
                    if (retryAttempt <= maxRetries) {
                        console.log(chalk_1.default.yellow(`⚠️  Attempt ${retryAttempt - 1} failed: ${retryError instanceof Error ? retryError.message : 'Unknown error'}`));
                        const { continueRetry } = await inquirer_1.default.prompt([{
                                type: 'confirm',
                                name: 'continueRetry',
                        ...
                        console.log(chalk_1.default.red(`❌ All retry attempts failed. Last error: ${retryError instanceof Error ? retryError.message : 'Unknown error'}`));
                        throw retryError;
                    }
                }
            }
