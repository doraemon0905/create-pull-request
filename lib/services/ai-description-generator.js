...
export class AIDescriptionGeneratorService {
    ...
    async selectAIProvider() {
        ...
        const preferredOrder = ['claude', 'chatgpt', 'gemini', 'copilot'];
    ...
    async generateSummary(options, provider) {
        ...
        // Confluence pages context if available
        if (jiraTicket.confluencePages && jiraTicket.confluencePages.length > 0) {
            ...
            for (const page of jiraTicket.confluencePages) {
                ...
                if (page.content) {
                    summaryPrompt += `- Content Summary: ${page.content}\n`;
                }
                summaryPrompt += `\n`;
            }
            summaryPrompt += `Please incorporate insights from these Confluence pages when generating the PR description, ensuring the implementation aligns with documented requirements and specifications.\n`;
        }
        ...
        summaryPrompt += `3. For EACH modified file, provides CONCISE detail in the File Changes Analysis section including:\n`;
        summaryPrompt += `   - File header as clickable link: [src/filename.ext](GitHub_URL)\n`;
        summaryPrompt += `   - CONCISE explanation of what changed (2-3 sentences maximum)\n`;
        summaryPrompt += `   - How the change maps to JIRA ticket requirements\n`;
        summaryPrompt += `   - Key functions/methods/classes that were modified, added, or removed\n`;
        summaryPrompt += `   - Why the change was necessary to fulfill JIRA requirements\n`;
        summaryPrompt += `   - Core code patterns added/removed (from the provided diffs)\n`;
        summaryPrompt += `   - Multiple specific line links for significant changes\n`;
    ...
    buildPrompt(options, summary) {
        ...
        if (jiraTicket.confluencePages && jiraTicket.confluencePages.length > 0) {
            ...
            for (const page of jiraTicket.confluencePages) {
                ...
                if (page.content) {
                    prompt += `- Key Information: ${page.content}\n`;
                }
                prompt += `\n`;
            }
            prompt += `CRITICAL: Use the information from these Confluence pages to:\n`;
            prompt += `1. Validate that the code changes align with documented requirements\n`;
            prompt += `2. Reference relevant specifications or design decisions in the PR description\n`;
            prompt += `3. Ensure the implementation follows established patterns and guidelines mentioned in the documentation\n`;
            prompt += `4. Include relevant context from the documentation that helps reviewers understand the requirements\n`;
        }
        prompt += `\n`;
        ...
        if (template) {
            ...
            prompt += `5. Fill template sections with DETAILED information including:\n`;
            prompt += `   - CONCISE explanations of all code changes (2-3 sentences per file maximum)\n`;
            prompt += `   - How each code change fulfills specific JIRA ticket requirements\n`;
            prompt += `   - Key functions/methods/classes modified with brief analysis\n`;
            prompt += `   - Technical implementation approach and integration points\n`;
            prompt += `   - Business value and impact on users/system\n`;
            prompt += `   - Connection between implementation and JIRA ticket acceptance criteria\n`;
            ...
            prompt += `   - Explains how each code change fulfills specific JIRA ticket requirements\n`;
            prompt += `   - References specific files and line numbers for significant changes\n`;
            prompt += `   - Describes the relevance of file changes to the overall solution\n`;
            prompt += `   - Connects implementation details to the JIRA ticket description and requirements\n`;
            prompt += `   - Analyzes the before/after state for modified code and why changes were necessary\n`;
        ...
        prompt += `- Provide concise explanations (2-3 sentences maximum) for each file modification\n`;
        prompt += `- Include specific function/method names and their purposes\n`;
        prompt += `- Explain the technical approach, architecture decisions, and integration points\n`;
        prompt += `- Map code changes to corresponding JIRA ticket requirements\n`;
        prompt += `- Explain why each change was implemented and how it solves the ticket problem\n`;
        prompt += `- Include before/after analysis for modified code sections\n\n`;
    ...
    async callClaudeAPI(client, prompt) {
