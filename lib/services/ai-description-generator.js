...
class AIDescriptionGeneratorService {
    ...
    initializeClients() {
        ...
        if (claudeKey) {
            this.clients.set('claude', axios_1.default.create({
                baseURL: constants_1.API_URLS.CLAUDE_BASE_URL,
                headers: {
    ...
    async generatePRDescription(options) {
        try {
            ...
            console.log('Selected AI provider:', this.selectedProvider);
    ...
    getAIProvidersConfig() {
        try {
            return (0, config_1.getConfig)('aiProviders');
        }
        catch {
            return null;
        }
    ...
    getModelForProvider(provider) {
        ...
        switch (provider) {
            case 'claude':
                return aiProvidersConfig?.claude?.model || process.env.CLAUDE_MODEL || constants_1.DEFAULT_MODELS.CLAUDE;
            case 'chatgpt':
                return aiProvidersConfig?.openai?.model || process.env.OPENAI_MODEL || constants_1.DEFAULT_MODELS.OPENAI;
            case 'gemini':
                return aiProvidersConfig?.gemini?.model || process.env.GEMINI_MODEL || constants_1.DEFAULT_MODELS.GEMINI;
            case 'copilot':
                return aiProvidersConfig?.copilot?.model || process.env.COPILOT_MODEL || constants_1.DEFAULT_MODELS.COPILOT;
            default:
